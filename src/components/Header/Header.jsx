...
const Navbar = ({ activePage = "/" }) => {
  ...
  const navItems = [
    ...
    { name: "Sobre", path: "/about", icon: <UserIcon className="w-4 h-4" /> },
    { name: "Projetos", path: "/projects", icon: <FolderIcon className="w-4 h-4" /> },
    { name: "Habilidades", path: "/skills", icon: <CodeBracketIcon className="w-4 h-4" /> },
    { name: "Experiência", path: "/experience", icon: <BriefcaseIcon className="w-4 h-4" /> },
    { name: "Contato", path: "/contact", icon: <EnvelopeIcon className="w-4 h-4" /> }
  ];
  ...
  return (
    <motion.header
      ...
      initial="hidden"
      animate="visible"
      className={`fixed top-0 w-full z-50 transition-all duration-300 ease-in-out hover:scale-105 ${
        scrolled 
            ...
            <Link href="/" legacyBehavior>
              <a className="flex items-center gap-2 group">
                <div className="hidden sm:block">
                  <h2 className="text-sm font-semibold text-white group-hover:text-primary-300 transition-colors">Portfolio</h2>
                  <span className="text-xs text-[#909dda]">Desenvolvedor Front-end</span>
                </div>
              </a>
            </Link>
          </div>
            ...
            >
                    ...
                    <Link href={item.path} legacyBehavior>
                      <a 
                        className={`flex items-center px-3 py-2 text-sm font-medium rounded-full transition-all ${
                          isActive 
                            ? "text-white bg-primary-500/10 border-l-2 border-purple-padrao" 
              ...
              <motion.li variants={itemVariants} className="ml-3">
                <a 
                  ...
                  className="flex items-center px-4 py-2 text-sm font-medium rounded-full bg-primary-500 text-white hover:bg-primary-600 transition-all transform hover:scale-105"
                >
          ...
          >
            <div className="px-2 py-3 space-y-1">
              ...
              <a 
                ...
                target="_blank" 
                rel="noopener noreferrer"
                className="flex items-center px-3 py-2 text-base font-medium rounded-lg text-white bg-primary-500/80 hover:bg-primary-500"
              >
